...
add{Section}()                           // Add new item
update{Section}(index, field, value)     // Update item field
remove{Section}(index)                   // Remove item
move{Section}(fromIndex, toIndex)        // Reorder items

// Achievement operations (Experience/Volunteering)
add{Section}Achievement(sectionIndex, achievement?)
update{Section}Achievement(sectionIndex, achievementIndex, text)
remove{Section}Achievement(sectionIndex, achievementIndex)
move{Section}Achievement(sectionIndex, fromIndex, toIndex)
```
